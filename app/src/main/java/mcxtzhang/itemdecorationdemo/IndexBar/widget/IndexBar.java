...
public class IndexBar extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        int textSize = (int) TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_SP, 4, getResources().getDisplayMetrics());//默认的TextSize
        mPressedBackground = R.color.colorPrimary;//默认按下是纯黑色
...
/*            switch (attr) {
     ...
     * @return
     */
    private void initSourceDatas() throws PinyinException {
        //add by zhangxutong 2016 09 08 :解决源数据为空 或者size为0的情况,
...
//            for (int i1 = 0; i1 < target.length(); i1++) {
            ...
            String s = PinyinHelper.convertToPinyinString(target, "", PinyinFormat.WITHOUT_TONE);
            pySb.append(s.toUpperCase());
